...
// ─── Quotation status derived from fields ─────────────────────────────────────
const getQuotStatus = (q) => {
  if (q.accepted) return "ACCEPTED";
  if (q.sended) return "SENT";
  return "DRAFT";
};
...
const PreSaleDetailPopup = ({ item, onClose }) => {
  ...
  const { mutate: createQuotation, isPending: creatingQuot } =
    useCreateQuotation();
  const { mutate: acceptQuotation } = useMarkQuotationAccepted();
  ...
  const handleAddQuotation = () => {
    if (!quotForm.title || !quotForm.amount) return;
    const t = showLoading("Adding quotation...");
    createQuotation(
      { preSalesId: item.srNumber, quotation: quotForm },
      {
        onSuccess: () => {
          dismissToast(t);
          showSuccess("Quotation added");
          setShowQuotationForm(false);
          setQuotForm({
            title: "",
            amount: "",
          ...
          {activeTab === "quotations" && (
            <div className={styles.quotSection}>
              ...
              {isQuotLoading ? (
                ...
                <div className={styles.quotList}>
                  {quotations.map((q, i) => {
                    const status = getQuotStatus(q);
                    const cfg = QUOT_STATUS_CFG[status];
                    const isAccepted = q.accepted === true;

                    return (
                      <div
                        key={q.id || i}
                        className={`${styles.quotCard} ${isAccepted ? styles.quotCardAccepted : ""}`}
                        onClick={() => handleCardClick(q.id)}
                      >
                        {/* left accent */}
                        <div
                          className={styles.quotCardAccent}
                          style={{ background: cfg.dot }}
                        />

                        {/* Main content */}
                        <div className={styles.quotCardBody}>
                          {/* Row 1 — number + status */}
                          <div className={styles.quotCardRow1}>
                            <div className={styles.quotCardLeft}>
                              <span className={styles.quotCardIndex}>
                                #{i + 1}
                              </span>
                              <div>
                                <div className={styles.quotCardNumber}>
                                  {q.quotationNumber || `QUOTE-${q.id}`}
                                </div>
                                {q.quotationDetails && (
                                  <div className={styles.quotCardDetails}>
                                    {q.quotationDetails}
                                  </div>
                                )}
                              </div>
                            ...
                            >
                              <span
                                className={styles.quotStatusDot}
                                style={{ background: cfg.dot }}
                              />
                              {cfg.label}
                            </span>
                          ...
                          <div className={styles.quotCardMeta}>
                            {q.budget != null && (
                              <span className={styles.quotMetaChip}>
                                💰 {fmtCurrency(q.budget)}
                              </span>
                            )}
                            <span className={styles.quotMetaChip}>
                              🗓 {fmt(q.dateTimeIssued)}
                            </span>
                            <span
                              className={styles.quotMetaChip}
                              style={
                                q.sended
                          ...
                          {/* Row 3 — actions */}
                          <div
                            className={styles.quotCardActions}
                            onClick={(e) => e.stopPropagation()}
                          >
                            <button
                              className={styles.quotViewBtn}
                              onClick={() => handleCardClick(q.id)}
                            >
                              View Details →
                            </button>
            ...
            <tbody>
              {data?.map((item, i) => (
                <tr key={item.srNumber}>
                  ...
                  <td>
                    <span
                      className={`${styles.badge} ${item.status === "Onboarded" ? styles.badgeSuccess : styles.badgeDanger}`}
                    >
                      {item.status}
                    </span>
                  </td>
      ...
      {isModalOpen && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalBox}>
            <h3>Convert to Project</h3>
            <p>
              Are you sure you want to convert this PreSales into a Project?
            </p>
            <div className={styles.modalActions}>
              <button
                className={styles.cancelBtn}
