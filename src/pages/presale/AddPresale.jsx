...
const PreSalesPage = () => {
  ...
  /* ===============================
     ...
     Body: PreSales { client, personName, approachedVia, conclusion, quotations }
     
     When existingClient=false → backend does clientRepository.save(client)
       so client body must include: { name, email, phone, address, password }
       password is saved directly (⚠ backend should encode — check your PreSales service)
     
     When existingClient=true → backend uses client.id to look up existing client
       so client body only needs: { id }
  =============================== */
  const handleSubmit = () => {
    // Phone validation for new client
    if (!isExistingClient) {
      ...
      // Password validation for new client
      if (!formData.client?.password?.trim()) {
        showError("Password is required for new client.");
        return;
      }
      if (formData.client.password.length < 6) {
    ...
    mutate(
      { data: payload, existingClient: isExistingClient },
      {
        onSuccess: () => {
          dismissToast(loadingToast);
          showSuccess("PreSales Created Successfully");
        ...
        onError: (error) => {
          ...
          console.log("RESPONSE:", error.response?.data);
          showError(
            error.response?.data?.message || "Failed to create PreSales",
          );
        },
      },
    );
          ...
          <label className={styles.checkboxWrapper}>
            <input
              ...
              onChange={() => {
                ...
                // Reset client data when switching modes
        ...
        <div className={styles.gridTwo}>
          {!isExistingClient ? (
            ...
            <>
              ...
              {/* ── Password — required for new client portal login ── */}
              {/* Sent in client body; backend saves via clientRepository.save()  */}
              <div className={styles.formGroup}>
                ...
                <input
                  className={styles.input}
                  type="text"
                  placeholder="Client portal login password (min. 6 chars)"
        ...
        <div className={styles.gridTwo}>
          ...
          <div className={styles.formGroup}>
            ...
            <select
              ...
              value={formData.approachedVia}
              onChange={(e) =>
                setFormData({ ...formData, approachedVia: e.target.value })
              }
            >
              <option value="">Select</option>
              <option value="Call">Call</option>
